...
if ($mybb->input['action'] == 'add' || $mybb->input['action'] == 'edit') {
    ...
    foreach (['groups', 'visible', 'forums', 'scripts', 'frules', 'style'] as $key) {
    ...
    $style_checked = ['default' => '', 'custom' => ''];
    if ($mybb->get_input('style_type') == 'default' || in_array(
            $mybb->get_input('style'),
            $annbars->styles
        ) || ($add && $mybb->request_method != 'post')) {
    ...
    $visible_checked = ['everywhere' => '', 'custom' => ''];
    if ($mybb->get_input('visible_type') == 'everywhere' || ($mybb->request_method != 'post' && $mybb->get_input(
                'visible',
                1
            ) === 1) || ($add && $mybb->request_method != 'post')) {
    ...
    if ($mybb->get_input('forums_type') == 'all' || $mybb->get_input(
            'forums',
            1
        ) == -1 || ($add && $mybb->request_method != 'post')) {
    ...
    } elseif ($mybb->get_input('forums_type') == 'none' || $mybb->get_input('forums') == '' && !$mybb->get_input(
            'forums',
            2
        )) {
    ...
    if ($add) {
        ...
        $form = new Form($annbars->build_url(['action' => 'edit', 'aid' => $bar['aid']]), 'post');
        $form_container = new FormContainer($sub_tabs['ougc_annbars_edit']['description']);
    }

    $form_container->output_row(
        $lang->ougc_annbars_form_name . ' <em>*</em>',
        $lang->ougc_annbars_form_name_d,
        $form->generate_text_box('name', $annbars->bar_data['name'])
    );
    $form_container->output_row(
        $lang->ougc_annbars_form_content . ' <em>*</em>',
        $lang->ougc_annbars_form_content_d,
        $form->generate_text_area(
            'content',
            $annbars->bar_data['content'],
            ['rows' => 10, 'cols' => 90, 'style' => 'width: auto;']
        )
    );
		...
		<dd style=\"margin-top: 4px;\" id=\"style_forums_groups_default\" class=\"style_forums_groups\">
			<table cellpadding=\"4\">
				<tr>
					...
					<td>" . $form->generate_select_box('style_picker', [
    ...
    $form_container->output_row(
        $lang->ougc_annbars_form_style,
        $lang->ougc_annbars_form_style_d,
        $style_select,
        '',
        [],
        ['id' => 'row_style']
    ...
    $form_container->output_row(
        $lang->ougc_annbars_form_groups,
        $lang->ougc_annbars_form_groups_d,
        $groups_select,
        '',
        [],
        ['id' => 'row_groups']
		...
		<dd style=\"margin-top: 4px;\" id=\"visible_forums_groups_custom\" class=\"visible_forums_groups\">
			<table cellpadding=\"4\">
				...
				<tr>
					...
					<td>" . $form->generate_text_area(
            'scripts',
            $annbars->bar_data['scripts'],
            ['rows' => 7, 'cols' => 60, 'style' => 'width: auto;']
        ) . "</td>
				</tr>
			</table>
		</dd>
    ...
    $form_container->output_row(
        $lang->ougc_annbars_form_visible,
        $lang->ougc_annbars_form_visible_d,
        $visible_select,
        '',
        [],
        ['id' => 'row_visible']
    ...
    $form_container->output_row(
        $lang->ougc_annbars_form_dismissible,
        $lang->ougc_annbars_form_dismissible_d,
        $form->generate_yes_no_radio('dismissible', $annbars->bar_data['dismissible'])
    );
    $form_container->output_row(
        $lang->ougc_annbars_form_startdate . ' <em>*</em>',
        $lang->ougc_annbars_form_startdate_d,
        $form->generate_date_select(
            'startdate',
            $annbars->bar_data['startdate_day'],
            $annbars->bar_data['startdate_month'],
            $annbars->bar_data['startdate_year']
        )
    );
    $form_container->output_row(
        $lang->ougc_annbars_form_enddate . ' <em>*</em>',
        $lang->ougc_annbars_form_enddate_d,
        $form->generate_date_select(
            'enddate',
            $annbars->bar_data['enddate_day'],
            $annbars->bar_data['enddate_month'],
            $annbars->bar_data['enddate_year']
        )
    );

    $form_container->output_row(
        $lang->ougc_annbars_form_frules . ' <em>*</em>',
        $lang->ougc_annbars_form_frules_d,
        $form->generate_text_area(
            'frules',
            $annbars->bar_data['frules'],
            ['rows' => 10, 'cols' => 90, 'style' => 'width: auto;']
        )
    );
    ...
    $form->output_submit_wrapper(
        [$form->generate_submit_button($lang->ougc_annbars_button_submit), $form->generate_reset_button($lang->reset)]
    );
    ...
    $page->output_confirm_action(
        $annbars->build_url(
            ['action' => 'delete', 'aid' => $mybb->input['aid'], 'my_post_key' => $mybb->post_code]
        )
    );
    ...
    $form_container->output_row_header($lang->ougc_annbars_form_name, ['width' => '20%']);

    $form_container->output_row_header($lang->ougc_annbars_form_content);

    $form_container->output_row_header($lang->ougc_annbars_form_status, ['width' => '10%', 'class' => 'align_center']);

    $form_container->output_row_header($lang->ougc_annbars_form_order, ['width' => '10%', 'class' => 'align_center']);

    $form_container->output_row_header($lang->options, ['width' => '10%', 'class' => 'align_center']);
    ...
    if ($barscount < 1) {
        $form_container->output_cell(
            '<div align="center">' . $lang->ougc_annbars_view_empty . '</div>',
            ['colspan' => 4]
        );
        ...
        while ($bar = $db->fetch_array($query)) {
            $editurl = $annbars->build_url(['action' => 'edit', 'aid' => $bar['aid']]);
            ...
            $form_container->output_cell(ougc_getpreview($bar['content'], 350, true, true, ['allow_html' => 1]));

            $form_container->output_cell(
                '<img src="../' . $config['admin_dir'] . '/styles/default/images/icons/bullet_' . $bar['visible'] . ($mybb->version_code >= 1800 ? '.png' : '.gif') . '" alt="' . $lang->{$bar['lang']} . '" title="' . $lang->{$bar['lang']} . '" />',
                ['class' => 'align_center']
            );
            ...
            $form_container->output_cell(
                $form->generate_numeric_field(
                    "disporder[{$bar['aid']}]",
                    $bar['disporder'],
                    ['min' => 0, 'class' => 'align_center', 'style' => 'width:80%;']
                ),
                ['class' => 'align_center']
            );
            ...
            $popup->add_item(
                $lang->ougc_annbars_tab_preview,
                $annbars->build_url(
                    ['action' => 'preview', 'aid' => $bar['aid']]
                )
            );
            $popup->add_item($lang->delete, $annbars->build_url(['action' => 'delete', 'aid' => $bar['aid']]));
    ...
    // Multipage
    if ($multipage = trim(
        draw_admin_pagination($mybb->input['page'], $perpage, $barscount, $annbars->build_url(false, 'page'))
    )) {
        echo $multipage;
    }
